/* Reusable Tweaks shell + form-control helpers (ported to TS).
 *
 * Listens for the edit-mode host protocol (__activate_edit_mode /
 * __deactivate_edit_mode) so it keeps working when embedded in a host, and
 * also exposes a standalone floating toggle so it is usable on its own. */
...
const __TWEAKS_STYLE = `
  .twk-fab{position:fixed;right:16px;bottom:16px;z-index:2147483645;width:42px;height:42px;
    border-radius:12px;border:.5px solid rgba(255,255,255,.6);cursor:pointer;
    background:rgba(250,249,247,.78);color:#29261b;font-size:18px;line-height:1;
...
export function TweakRadio({ label = '', value, options, onChange }: TweakRadioProps) {
  ...
  const opts: RadioOption[] = options.map((o) =>
